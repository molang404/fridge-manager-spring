...
public class UserService {
    ...
    @Transactional // 진행 중간에 에러가 만약 발생되면 DB 작업한 것을 롤백하는 어노케이션
    public User createUser(CreateUserRequest request) {
        // 우리가 Express를 사용할 때에는 schema.prisma만 만들고,
        // generate를 실행하면 우리가 사용할 수 있는 모든 메서드가 작성되어서 만들어지고 사용만 하면 됐음

        // Spring-Boot, Hibernate에서는 메서드를 자동으로 만들어주는 건 findById (PrimaryKey를 대상으로 검색)만 있고
        ...
        // 입력된 birthdate는 String이니까 이걸 Date 객체로 변환
        LocalDate parseBirthdate = null;
        // 사용자가 입력한 birthdate가 null이 아니면서 빈값("")도 아닐 경우
        if (request.getBirthdate() != null && !request.getBirthdate().isBlank()) {
            parseBirthdate = LocalDate.parse(request.getBirthdate(), DateTimeFormatter.ISO_DATE);
        }
