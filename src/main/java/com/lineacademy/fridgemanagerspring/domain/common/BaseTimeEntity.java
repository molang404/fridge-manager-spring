...
@Getter // 클래스 내부에 모든 멤버 변수들의 Getter 메서드를 자동 생성
// Mapping : ORM에서 테이블과 클래스(모델)을 연결짓는 과정
// 그러한 맵핑을 할 때 기반이 되는 자식 클래스들의 부모로서 작용한다고 지정
// Entity -> 테이블과 연결되는 클래스들을 지칭
// MappedSuperclass는 DB와 연관성ㅇ르 갖는 클래스는 맞지만, 이걸로 테이블 연결은 안하겠다
@MappedSuperclass
